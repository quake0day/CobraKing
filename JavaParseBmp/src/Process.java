...
public class Process {
	...
	public BufferedImage mImageOrig = null;
	public BufferedImage mImageCpy = null;
	...
	public void initProcess()
	{
		...
		System.out.println("H:" + Integer.toString(bmpHeight)+", W:" + Integer.toString(bmpWidth));
    ...
    {
    	/*int dob = computeDOB(mImageOrig);
    	
    	if(dob>70)
    		return true;
    	
    	polarizeImageHSV(mBmp, pixels);*/
    ...
    private int computeDOB(BufferedImage mBmp)
    {
    	int sum = 0;
    	int cnt = 0;
    	int t,c;
    	...
    	for (int x = 0; x<bmpWidth; x++)
    		for(int y = 0; y<bmpHeight; y++)
    		{
    			...
    			red = ColorConverter.red(c);
    			green = ColorConverter.green(c);
    			blue = ColorConverter.blue(c);
    			 
    			
    			if(Math.abs(255-red)>red)
    				t+=red;
    			else
    				t+=Math.abs(255-red);
    			
    			if(Math.abs(255-green)>green)
    				t+=green;
    			else
    				t+=Math.abs(255-green);
    			
    			if(Math.abs(255-blue)>blue)
    				t+=blue;
    			else
    				t+=Math.abs(255-blue);
    	...
    	//mTextView.append("@DOB:"+Integer.toString(sum/cnt));
    	
    	return (int)((sum/cnt)/3);
    }
    ...
    private void saveImage(int[] buf)
    {
    	BufferedImage bi =
                new BufferedImage(bmpWidth, bmpHeight, BufferedImage.TYPE_INT_ARGB);
    	bi.setRGB(0, 0, bmpWidth, bmpHeight, buf, 0, bmpWidth);
    	
    	File outputfile = new File("C:/Users/Tian/Desktop/saved.png");
